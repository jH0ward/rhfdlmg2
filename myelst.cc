...
MyElstInt::MyElstInt(std::vector<SphericalTransform>& st, std::shared_ptr<BasisSet> bs1, std::shared_ptr<BasisSet> bs2, int deriv) :
    ElectrostaticInt(st,bs1,bs2,deriv)
{
  //std::cout << "Creating elst_int\n";
}
...
void MyElstInt::compute(SharedMatrix &result, const Vector3& C)
{
    // Do not worry about zeroing out result
    //std::cout <<  "i'm in compute\n";
    int ns1 = bs1_->nshell();
    int ns2 = bs2_->nshell();
    ...
    //#pragma omp parallel for private(i_offset,location)
    for (int i=0; i<ns1; ++i) {
...
void MyElstInt::compute_shell(int sh1, int sh2, const Vector3& C)
{
...
void MyElstInt::compute_pair(const GaussianShell& s1, const GaussianShell& s2, const Vector3& C)
{
    int ao12;
    int am1 = s1.am();
    int am2 = s2.am();
...
SharedVector MyElstInt::nuclear_contribution(std::shared_ptr<Molecule> mol)
{
